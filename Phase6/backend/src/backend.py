...
class Backend:
    ...
    def __init__(self, users_filepath: str, items_filepath: str, merged_daily_transactions_filepath: str):
        self.transactions = []
        self.users = []
        self.items = []
        ...
        self.merged_daily_transactions_filepath = merged_daily_transactions_filepath
    ...
    def find_user(self, username) -> User:
        found_users = [user for user in self.users if user.username == username]
        return found_users[0] if found_users else None
    
    def find_item(self, item_name, seller) -> Item:
        found_items = [item for item in self.items if item.item_name == item_name and item.seller == seller]
        return found_items[0] if found_items else None
    
    def set_available_users(self) -> None:
        file = open(self.users_filepath, "r")
    ...
    def set_available_items(self) -> None:
        file = open(self.items_filepath, "r")
    ...
    def set_merged_daily_transactions(self) -> None:
        ...
        file = open(self.merged_daily_transactions_filepath, "r")
        for line in file:
            ...
            if transaction_code == "01":
                ...
                new_user = None
                if privilege == "AA":
                    new_user = Admin(token[1], 'password', float(token[3]))
                elif privilege == "FS":
                    new_user = FullStandard(token[1], 'password', float(token[3]))
                elif privilege == "BS":
                    new_user = BuyStandard(token[1], 'password', float(token[3]))
                elif privilege == "SS":
                    new_user = SellStandard(token[1], 'password', float(token[3]))
    ...
    def write_new_available_users(self) -> None:
        file = open(self.users_filepath, "w")
    ...
    def write_new_available_items(self) -> None:
        file = open(self.items_filepath, "w")
    ...
    def process(self) -> None:
        ...
        # writes new available users and items to file
        self.write_new_available_users()
        self.write_new_available_items()
